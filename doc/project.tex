...
\begin{table}[ht]
\centering
\caption{Disease stages of the Ratingen-score \cite{rau_2007} }
\label{tab:ratingen}
\begin{tabular}{@{}ll@{}}
\toprule
...
\end{tabular}
\caption{Training and validation loss and accuracy for the three classification models.}
\label{fig:loss_acc_clas}
\end{figure}

Model b) was trained for the full 25 epochs, whereas the other two models started overfitting and had to be stopped early after 9 epochs. The training and validation loss and accuracy for the three models are shown in \autoref{fig:loss_acc_clas}. \autoref{fig:confusion} shows the normalized confusion matrices for the validation dataset for the three models.
...
\end{tabular}
\caption{Normalized confusion matrices for the predictions of the classifcation models on the validation set.}
\label{fig:confusion}
\end{figure}
...
As a second metric, we calculated the normalized accuracy for predictions that are in the right class or no more than one class above or below the correct class. This metric takes into account, that a misclassification by 1 is far less severe than a misclassification by 5.
...
The blue area visualized in \autoref{fig:crps} shows approximately what the CRPS is. \cite{crps}
...
The model predicts the discrete cdf for the percentage of bone erosion. An example of a prediction is shown by the blue dots in \autoref{fig:crps_pred}. The green line shows the true label whereas the red line shows the expected value of the predicted distribution. The expected value of our discrete cdf is calculated as follows.

$$E(x) = \sum_{i=1}^{101} \left ( 1 - p_i \right )$$

Where $p_i$ are the 101 predictions.
...
This model was trained in the same ways as described in \autoref{subsubsec:clas} on the original data, oversampled data and with a weighted loss function. This time all three models were trainer for 25 epochs, as they did not start overfitting. The train and validation loss and mean absolute error (mae) can be seen in \autoref{fig:loss_acc_reg}. The mae is calculated for the predictions of the cdf and not for the final labels.\autoref{fig:hexbin} shows the predictions for the validation set in all three cases. 
...
\end{tabular}
\caption{Training and validation loss and mean absolute error for the three regression models.}
...
\end{tabular}
\caption{Predictions of the regression models on the validation set.}
\label{fig:hexbin}
\end{figure}
...
In contrast to \autoref{fig:loss_acc_reg}, where mae is calculated for the predictions of the cdf, the mse and mae in the evaluation metrics in \autoref{tab:metrics_reg} are calculated for the final predictions. We can see, that model a) has a smallest mean squared error (mse) as well as the smallest mean absolute error (mae) of the three models. Therefore we decided to use model a) as the base model for regression.
...
\end{tabular}
\caption{Training and validation loss and accuracy for the transfer learning regression model.}
...
We have now found the best models for classification and regression. Those two models are now validated on the validation set. \autoref{fig:eval} shows the predictions of the two models for the validation set.
...
\end{tabular}
\caption{Predictions of the best two models for the test set}
\label{fig:eval}
\end{figure}

The evaluation metrics for the predictions of the two models for the validation set are shown in \autoref{tab:eval_metrics}.
...
\end{tabular}
\end{tabular}
\caption{Evaluation metrics for the best models evaluated for the validation set}
\label{tab:validation_metrics}
\end{table}

The evaluation metrics of the classification model for the test set are very similar to the ones for the validation set. The classification model performs equally well on the validation and test set. The regression model however performed worse on the test set. Both, the MSE and MAE are higher for the test set than the validation set. This could indicate that the regression model is overfitting.

\subsection{Comparing the classification and regression model}
\label{subsec:comparison_clas_reg}

In order to compare the regression model to the classification model the predicted percentages of joint erosion were converted to Ratingen scores. Because the predictions are never exactly zero, we increased the range of class 0 to percentage of bone erosion between 0 and 1. Class 1 is therefore from 1 \% to 10 \% bone erosion. All other classes are  according to \autoref{subsec:ratingen}. The resulting confusion matrix is shown in \autoref{fig:comparison} and the corresponding evaluation metrics can be seen in \autoref{tab:comparison}.
...
\end{tabular}
\caption{Confusion matrix of predictions of the regression model converted to classes.}
\label{fig:comparison}
\end{figure}

When comparing the confusion matrix for the validation set to c) in \autoref{fig:confusion} and the confusion matrix for the test set to a) in \autoref{fig:eval}, we can see that the regression model is more accurate for the labels 0, 1 and 5 whereas the accuracy for the labels 2, 3 and 4 is lower. It also seems that are a more outliers compared to the classification model, for example observations with the true label 5 that are mistakenly predicted as label 1.
...
The evaluation metrics are compared to c) in \autoref{tab:metrics_cla} for the validation set and \autoref{tab:eval_metrics} for the test set. For both the validation and the test set the accuracy of the regression model is higher, since the overrepresented class 1 is predicted more accurately. Whereas the class normalized accuracies are slightly lower than for the classification model.
...
In order to see where the attention of the models lies, the outputs of intermediate convolutional layers were visualized. Ideally, the last convolutional layer of the model should be visualized. However, the last convolutional layer in our classification model has the dimensions 9 x 9 which is a very low resolution and would lead to a very coarse attention map. Therefore the last convolutional layer with higher dimensions was chosen, its outputs have a dimension of 18 x 18. In the transfer learning regression model, a similar resolution was desired. But the outputs of the last convolutional layer with the dimension 16 x 16 showed attention all over the image. Therefore we had to use a convolutional layer closer to the end of the model. The last convolutional layer with dimension 7 x 7 was chosen, which is very coarse, but shows a centralized attention.

The outputs of every filter of the chosen convolutional layers was added together and normalized to values in the range [0, 1]. It was then colormapped and overlaid onto the original image. Yellow shows high attention whereas purple shows low attention.

\autoref{fig:attention} shows that the attention of both models focuses on the gap between the two bones most of the time. This is what was desired, since the bone erosion happens in that area.
...
\end{tabular}
\caption{Attention maps of the best two models for random samples of the validation set}
\label{fig:attention}
\end{figure}
...
\subsubsection{K-nearest-neighbor of embeddings}
\label{subsubsec:knn_embeddings}

By doing a k-nearest-neighbor (KNN) classification on the embeddings of the classification model we can see how well the model separates the different classes. The KNN classification achieved a normalized accuracy of 31 \% which is worse than the normalized accuracy achieved by the fully connected layers of the CNN of 42 \%. And also the normalized $\pm$ 1 accuracy is worse with 63 \% compared to 82 \%.

??????????? compare to classification transfer learning embeddings

The same can be done with a KNN regression for the embeddings of the regression model. Surprisingly, the KNN regression performs similarly well as the fully connected layers of the CNN. The MAE is a bit worse with 3.7 compared to 3.1, but the MSE is even better with 66.2 compared to 72.8. This means that the 

???????????? compare to regression base model embeddings

\subsubsection{Analysis of outliers in the embeddings}
\label{subsubsec:outliers_embeddings}

Next we analyzed the outliers in the embeddings of the regression model. We defined the area on the right side with majoritarian scores higher than 1 as the "bad area" whereas the area on the left side with majoritarian scores equal to zero is called the "good area". Our hypothesis is, that the rheumatologist is more likely to score a damaged joint lower if the other joints of this patient are healthy. The same reasoning can be applied in the reverse. The rheumatologist might be more likely to give a healthy joint a higher score if the other joints of this patient are damaged.

We defined all joints with a score $\leq$ 20 \% (Ratingen-score = 0) as healthy and all joints with a score $>$ 20 \% as damaged. In the "bad area" all healthy joints are seen as outliers whereas in the "good area" all damaged joints are seen as outliers \Cref{fig:outlier_embeddings} shows the mean score of patients which have a score that is an outlier compared to all scores that are not outliers.
...
\end{tabular}
\caption{Average score of patients which have a score that is an outlier.}
