...
Model b) was trained for the full 25 epochs, whereas the other two models started overfitting and had to be stopped early after 9 epochs. \autoref{fig:confusion} shows the normalized confusion matrices for the validation dataset for the three models.
...
We have now found the best models for classification and regression. Those two models are now validated on the validation set. \autoref{fig:validation} shows the predictions of the two models for the validation set.
...
The evaluation metrics for the predictions of the two models for the validation set are shown in \autoref{tab:validation_metrics}.
...
In order to compare the regression model to the classification model the predicted percentages of joint erosion were converted to Ratingen scores according to \autoref{subsec:ratingen}. The resulting confusion matrix is shown in \autoref{fig:comparison} and the corresponding evaluation metrics can be seen in \autoref{tab:comparison}.
...
The evaluation metrics are compared to c) in \autoref{tab:metrics_cla} for the validation set and \autoref{tab:validation_metrics} for the test set. For both the validation and the test set the accuracy of the regression model is higher, since the overrepresented class 1 is predicted more accurately. Whereas the class normalized accuracies are slightly lower than for the classification model.
...
In order to see where the attention of the models lies, the outputs of intermediate convolutional layers were visualized. \autoref{fig:attention} shows that the attention of both models focuses on the gap between the two bones most of the time. This is what was desired, since the bone erosion happens in that area.
