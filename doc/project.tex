...
The evaluation metrics of the classification model for the validation set are very similar to the ones for the test set. The classification model performs equally well on the test and validation set. The regression model however performed worse on the validation set. Both, the MSE and MAE are higher for the validation set than the test set. This could indicate that the regression model is overfitting.
...
\begin{figure}[ht]
\begin{tabular}{cc}
\subfloat[Test]{\includegraphics[width = 2.5in]{reg_clas_test}} &
\subfloat[Validation]{\includegraphics[width = 2.5in]{reg_clas_val}}
\end{tabular}
...
By doing a k-nearest-neighbor (KNN) classification on the embeddings of the classification model we can see how well the model separates the different classes. The KNN classification achieved a class normalized accuracy of 31 \% which is worse than the class normalized accuracy achieved by the fully connected layers of the CNN of 42 \%. And also the class normalized accuracy $\pm$ 1 is worse with 63 \% compared to 82 \%.
